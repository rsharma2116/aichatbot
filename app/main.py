...
# from app.services.gemini import query_gemini  # Changed import
...
#     try:
#         logger.info(f"Received chat request: {request.message}")
#         response = query_gemini(request.message)  # Changed function call
#         logger.info(f"AI response: {response}")
#         return {"reply": response}
#     except Exception as e:
#         logger.error(f"Chat error: {str(e)}")
...
from app.models import ChatRequest  # Import from models package
from app.utils import get_password_hash, verify_password, create_access_token
from fastapi.security import OAuth2PasswordBearer
from fastapi import status
...
def chat_with_ai(request: ChatRequest, token: str = Depends(oauth2_scheme)):
    try:
        logger.info(f"Received chat request: {request.message}")
        response = query_gemini(request.message)
        logger.info(f"AI response: {response}")
        return {"reply": response}
    except Exception as e:
        logger.error(f"Chat error: {str(e)}")
